...
var KTUsersUpdatePermissions = function () {
    ...
    var initUpdatePermissions = () => {
        ...
        closeButton.addEventListener('click', e => {
            ...
            Swal.fire({
                text: "آیا میخواهید پنجره را ببندید",
                icon: "warning",
...
function setAction(elem) {
    let formID = elem.getAttribute('data-target-form'); // example kt_modal_update_role_form
    console.log(formID);
    document.getElementById(formID).setAttribute('action', elem.getAttribute('data-action-url'));
}

function setName(elem) {
    let targetId = elem.getAttribute("data-target-value"); // example roleName
    document.getElementById(targetId).value = elem.getAttribute('data-target-name');
}

function resetCheckboxes(elem) {
    document.getElementsByClassName('form-check-input').forEach((item,index) => {item.checked=false;});
    JSON.parse(elem.getAttribute('data-permissions').trim().replace(',]',']')).forEach(function(item, index){document.querySelectorAll('input[value=\''+item+'\']').forEach(function(item2){item2.checked=true;});});
}
